...
use App\Filament\Clusters\Sales\Resources\Sales\ProformaResource\Pages;
use App\Filament\Clusters\Sales\Resources\Sales\ProformaResource\RelationManagers;
...
class ProformaResource extends Resource
{
            ...
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        ...
                        Forms\Components\Section::make('Proforma Invoice Items')
                            ->headerActions([
                                Action::make('reset')
                    ...
                    ])->columnSpan(['lg' => fn (?Proforma $record) => $record === null ? 3 : 2]),
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\Placeholder::make('created_at')
                                    ->label('Created at')
                                    ->content(fn (Proforma $record): ?string => $record->created_at?->diffForHumans()),

                                Forms\Components\Placeholder::make('updated_at')
                                    ->label('Last modified at')
                                    ->content(fn (Proforma $record): ?string => $record->updated_at?->diffForHumans()),
                            ])
                            ->columnSpan(['lg' => 1])
                            ->hidden(fn (?Proforma $record) => $record === null),
                        Forms\Components\Section::make()
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestInvoice = Proforma::latest('number')->first();

                        if ($latestInvoice) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('PINV-', '', $latestInvoice->number);
                            return 'PINV-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'PINV-000001'; // Default for the very first invoice
                    })
                    ->disabled()
                    ->label('Proforma invoice no')
                    ->dehydrated()
                    ->maxLength(32)
                    ->unique(Proforma::class, 'number', ignoreRecord: true)
                    ->maxLength(255),
            ...
            Forms\Components\Section::make('Attachment')
                ->schema([
                    SpatieMediaLibraryFileUpload::make('media')
                        ->collection('proforma-attachment')
                        ->multiple()
                        ->hiddenLabel(),
                ])
                ->collapsed(),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('proforma-attachment')
                    ->label('Attachment')

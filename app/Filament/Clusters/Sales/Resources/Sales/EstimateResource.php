...
use App\Filament\Clusters\Sales\Resources\Sales\EstimateResource\Pages;
use App\Filament\Clusters\Sales\Resources\Sales\EstimateResource\RelationManagers;
...
class EstimateResource extends Resource
{
            ...
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        ...
                        Forms\Components\Section::make('Estimate Invoice Items')
                            ->headerActions([
                                Action::make('reset')
                    ...
                    ])->columnSpan(['lg' => fn (?Estimate $record) => $record === null ? 3 : 2]),
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\Placeholder::make('created_at')
                                    ->label('Created at')
                                    ->content(fn (Estimate $record): ?string => $record->created_at?->diffForHumans()),

                                Forms\Components\Placeholder::make('updated_at')
                                    ->label('Last modified at')
                                    ->content(fn (Estimate $record): ?string => $record->updated_at?->diffForHumans()),
                            ])
                            ->columnSpan(['lg' => 1])
                            ->hidden(fn (?Estimate $record) => $record === null),
                        Forms\Components\Section::make()
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestInvoice = Estimate::latest('number')->first();

                        if ($latestInvoice) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('EST-', '', $latestInvoice->number);
                            return 'EST-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'EST-000001'; // Default for the very first invoice
                    })
                    ->disabled()
                    ->label('Estimate no')
                    ->dehydrated()
                    ->maxLength(32)
                    ->unique(Estimate::class, 'number', ignoreRecord: true)
                    ->maxLength(255),
    ...
    public static function getSummary():array
    {
        return [
            ...
            Forms\Components\Section::make('Attachment')
                ->schema([
                    SpatieMediaLibraryFileUpload::make('media')
                        ->collection('estimate-attachment')
                        ->multiple()
                        ->hiddenLabel(),
                ])
                ->collapsed(),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('estimate-attachment')
                    ->label('Attachment')

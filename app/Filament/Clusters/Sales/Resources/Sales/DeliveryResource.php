...
use App\Filament\Clusters\Sales\Resources\Sales\DeliveryResource\Pages;
use App\Filament\Clusters\Sales\Resources\Sales\DeliveryResource\RelationManagers;
...
class DeliveryResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'Delivery Challan';
            ...
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        ...
                        Forms\Components\Section::make('Delivery Challan Items')
                            ->headerActions([
                                Action::make('reset')
                    ...
                    ])->columnSpan(['lg' => fn (?Delivery $record) => $record === null ? 3 : 2]),
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\Placeholder::make('created_at')
                                    ->label('Created at')
                                    ->content(fn (Delivery $record): ?string => $record->created_at?->diffForHumans()),

                                Forms\Components\Placeholder::make('updated_at')
                                    ->label('Last modified at')
                                    ->content(fn (Delivery $record): ?string => $record->updated_at?->diffForHumans()),
                            ])
                            ->columnSpan(['lg' => 1])
                            ->hidden(fn (?Delivery $record) => $record === null),
                        Forms\Components\Section::make()
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestInvoice = Delivery::latest('number')->first();

                        if ($latestInvoice) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('DVL-', '', $latestInvoice->number);
                            return 'DVL-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'DVL-000001'; // Default for the very first invoice
                    })
                    ->disabled()
                    ->label('Delivery no')
                    ->dehydrated()
                    ->maxLength(32)
                    ->unique(Delivery::class, 'number', ignoreRecord: true)
                    ->maxLength(255),
            ...
            Forms\Components\Section::make('Attachment')
                ->schema([
                    SpatieMediaLibraryFileUpload::make('media')
                        ->collection('delivery-attachment')
                        ->multiple()
                        ->hiddenLabel(),
                ])
                ->collapsed(),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('dlivery-attachment')
                    ->label('Attachment')

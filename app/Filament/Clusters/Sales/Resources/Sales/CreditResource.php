...
use App\Filament\Clusters\Sales\Resources\Sales\CreditResource\Pages;
use App\Filament\Clusters\Sales\Resources\Sales\CreditResource\RelationManagers;
...
class CreditResource extends Resource
{
            ...
            ->schema([
                Forms\Components\Group::make()
                    ->schema([
                        ...
                        Forms\Components\Section::make('Credit Note Items')
                            ->headerActions([
                                Action::make('reset')
                    ...
                    ])->columnSpan(['lg' => fn (?Credit $record) => $record === null ? 3 : 2]),
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\Placeholder::make('created_at')
                                    ->label('Created at')
                                    ->content(fn (Credit $record): ?string => $record->created_at?->diffForHumans()),

                                Forms\Components\Placeholder::make('updated_at')
                                    ->label('Last modified at')
                                    ->content(fn (Credit $record): ?string => $record->updated_at?->diffForHumans()),
                            ])
                            ->columnSpan(['lg' => 1])
                            ->hidden(fn (?Credit $record) => $record === null),
                        Forms\Components\Section::make()
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestInvoice = Credit::latest('number')->first();

                        if ($latestInvoice) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('CRN-', '', $latestInvoice->number);
                            return 'CRN-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'CRN-000001'; // Default for the very first invoice
                    })
                    ->disabled()
                    ->label('Credit no')
                    ->dehydrated()
                    ->maxLength(32)
                    ->unique(Credit::class, 'number', ignoreRecord: true)
                    ->maxLength(255),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('credit-attachment')
                    ->label('Attachment')

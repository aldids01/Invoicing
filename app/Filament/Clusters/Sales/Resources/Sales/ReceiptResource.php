...
use App\Filament\Clusters\Sales\Resources\Sales\ReceiptResource\Pages;
use App\Filament\Clusters\Sales\Resources\Sales\ReceiptResource\RelationManagers;
...
class ReceiptResource extends Resource
{
        ...
        return $form
            ->schema([
                Forms\Components\Group::make()
                    ...
                    ])->columnSpan(['lg' => fn (?Receipt $record) => $record === null ? 3 : 2]),
                Forms\Components\Group::make()
                    ->schema([
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\Placeholder::make('created_at')
                                    ->label('Created at')
                                    ->content(fn (Receipt $record): ?string => $record->created_at?->diffForHumans()),

                                Forms\Components\Placeholder::make('updated_at')
                                    ->label('Last modified at')
                                    ->content(fn (Receipt $record): ?string => $record->updated_at?->diffForHumans()),
                            ])
                            ->columnSpan(['lg' => 1])
                            ->hidden(fn (?Receipt $record) => $record === null),
                        Forms\Components\Section::make()
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestInvoice = Receipt::latest('number')->first();

                        if ($latestInvoice) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('RPT-', '', $latestInvoice->number);
                            return 'RPT-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'RPT-000001'; // Default for the very first invoice
                    })
                    ->disabled()
                    ->label('Receipt no')
                    ->dehydrated()
                    ->maxLength(32)
                    ->unique(Receipt::class, 'number', ignoreRecord: true)
                    ->maxLength(255),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('receipt-attachment')
                    ->label('Attachment')

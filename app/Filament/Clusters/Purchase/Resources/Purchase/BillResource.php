...
class BillResource extends Resource
{
                        ...
                        Forms\Components\Section::make()
                            ->schema([
                                Forms\Components\Placeholder::make('created_at')
                                    ->label('Created at')
                                    ->content(fn (Bill $record): ?string => $record->created_at?->diffForHumans()),

                                Forms\Components\Placeholder::make('updated_at')
                                    ->label('Last modified at')
                                    ->content(fn (Bill $record): ?string => $record->updated_at?->diffForHumans()),
                            ])
                            ->columnSpan(['lg' => 1])
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestBill = Bill::latest('number')->first();

                        if ($latestBill) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('BLL-', '', $latestBill->number);
                            return 'BLL-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'BLL-000001'; // Default for the very first Bill
                    })
                    ->disabled()
                    ->label('Bill no')
                    ->dehydrated()
                ...
                Forms\Components\TextInput::make('po')
                    ...
                    ->unique(Bill::class, 'po', ignoreRecord: true)
                    ->required()
                    ->maxLength(32),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('bill-attachment')
                    ->label('Attachment')

...
use App\Filament\Clusters\Purchase\Resources\Purchase\OrderResource\Pages;
use App\Filament\Clusters\Purchase\Resources\Purchase\OrderResource\RelationManagers;
...
//    }
        ...
        return [
            ...
            Forms\Components\Group::make([
                Forms\Components\TextInput::make('number')
                    ->required()
                    ->default(function () {
                        $latestOrder = Order::latest('number')->first();

                        if ($latestOrder) {
                            // Extract the numeric part and increment
                            $lastNumber = (int) str_replace('PON-', '', $latestOrder->number);
                            return 'PON-' . sprintf('%06d', $lastNumber + 1); // Adjust %06d for your desired padding
                        }

                        return 'PON-000001'; // Default for the very first Order
                    })
                    ->disabled()
                    ->label('Order no')
                    ->dehydrated()
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\SpatieMediaLibraryImageColumn::make('order-attachment')
                    ->label('Attachment')
                    ->toggleable(isToggledHiddenByDefault: true)
                    ->collection('order-attachment'),
                Tables\Columns\TextColumn::make('number')
